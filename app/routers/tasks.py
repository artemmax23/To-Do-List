...
async def create_task(
    ...
    if task.tag_id:
        tag = await crud.get_tag(db, user.id, task.tag_id)
        if not tag:
            raise HTTPException(
    ...
    return await crud.create_task(db, user.id, task)
...
async def get_task(
    ...
    Response:
            ...
            ```
    """
    task = await crud.get_task(db, user.id, task_id)
...
async def update_task(
    ...
    Response:
            ...
            ```
    """
    existing_task = await crud.get_task(db, user.id, task_id)
   ...
   # Проверка существования задачи
    ...
    if task_data.tag_id:
        tag = await crud.get_tag(db, user.id, task_data.tag_id)
    ...
    return await crud.update_task(db, user.id, task_id, task_data)
...
async def delete_task(
    ...
    Response:
            ...
            ```
    """
    deleted = await crud.delete_task(db, user.id, task_id)
